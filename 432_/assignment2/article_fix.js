        ...
        if (result) {
            ...
            return axios.get(searchUrl)
                .then(async response => {
                    ...
                    console.log(result_array[0].title, "checking ****")
                    const target = await Article.findOne({ title: result_array[0].title })

                    if (target) {
                        console.log(" API result in the mongodb ")
                    } else {
                        console.log(" API result is not in mongodb")
                    }
